...
nyc_clean2 <- nyc_clean %>%
  ...
  mutate(transportation_desert_4num = factor(as.numeric(transportation_desert_4cat))) %>%
   ...
   mutate(below_poverty_perc = (below_poverty_line_count / total_pop) * 10) %>%
  ...
  mutate(nonwhite_perc = (10-white_perc)) %>%
  filter(nta_type == 0)
...
```{r}
model1 <- stan_polr(transportation_desert_4num ~ total_pop + mean_income + unemployment_perc + below_poverty_perc + evictions_perc + white_perc + asian_perc + black_perc + latinx_perc + school_count + store_count, data =nyc_clean2, prior=NULL,iter=500, seed = 86437, refresh=0, prior_PD=FALSE)
...
ordinal_accuracy(my_prediction1,nyc_clean2)
