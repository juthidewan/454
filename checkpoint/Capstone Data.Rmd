...
boundary_nyc <- st_read(here("ethnic", "Data", "zip_code_boundaries","ZIP_CODE_040114.shp"), 
                        quiet = TRUE) %>%
  st_transform(26915)
...
transit_points <- read_csv(here("transit","ridership_points.csv"))%>%
  ...
  mutate(latitude = as.numeric(latitude),
         longitude = as.numeric(longitude)) %>%
  mutate(search = paste(round(latitude,3), round(longitude,3), ".1", sep=","))%>%
  st_as_sf(coords = c("longitude", "latitude"), crs = 26915, agr = "constant")
...
st_bbox(transit_points)
st_bbox(boundary_nyc)

#plot locations over map
ggplot() +
  geom_sf(data = transit_points) + 
  geom_sf(data = nyc_join, fill = NA, color = "black", size = 0.15, alpha = 0) +
  coord_sf(datum = st_crs(26915)) +
  theme_minimal() +
  theme(panel.grid.major = element_line("transparent"),
        axis.text = element_blank()) +
  ggtitle("Ridership in NYC")+ 
    theme(#panel.grid.major = element_line("transparent"),
