...
- $y^{*}$ : a linear combination of 4 predictor variables consisting of `mean income`, `below poverty percentage`, `borough`, and `store_count`.
...
It seems our model is dramatically accurate when classifying neighborhoods with excellent subway access (72.22%), while entirely inaccurate when studying a neighborhood with satisfactory subway access (0.00%). For the remaining categories of transportation access, there was only a .02 difference in accuracy. Note that in both the ordinal and the naive model, we could not accurately predict this "Satisfactory" category. Likely, this must be a byproduct of our interval criteria for each respective level of transportation inaccess— that is, these categories may not be sufficiently different to find differences.
