...
Y_i| \zeta_1, \zeta_2,\zeta_3,\beta_1, \beta_2,\beta_3, \beta_4, \beta_5,\beta_6=
\begin{cases}
1 &, y^{*} < \zeta_1 \\
...
\\
y^{*} = \beta_1x_1 + \beta_2x_2 + \beta_3x_3 + \beta_4x_4 + \beta_5x_5 + \beta_6x_6 +  \epsilon \\

$$

- $\zeta_i$ : is a vector of cut-points for categories. 
- $y^{*}$ : a linear combination of 3 predictor variables consisting of `mean income`, `below poverty percentage`, `borough`, and `store_count`.
...
\beta_5 \sim N(m_{\beta 5}, s_{\beta 5}^{2}) \\
\beta_6 \sim N(m_{\beta 6}, s_{\beta 6}^{2}) \\
$$

Each $\zeta_i$ is a vector of cut-points for categories, and each $\beta_i$ is the coefficient for the $i^{th}$ predictor variable. 

Since we do not have prior information about the data, we will be using the default prior R2 given by the `stan_polr` function, which is null, or a uniform prior. We would also use a default prior count dirichlet(1), which is a uniform prior.

Note:
- There is no intercept in this model because the data cannot distinguish an intercept from the cut points. 

One challenge we face when creating our ordinal model is that the Bayesian ordinal regression does not do cross-validated checks like other Bayesian regression or classification. To test the model's fitness into outside data and prevent overfitting, we used a train-test split. Our train data would include 70% of the original observations and our test data would include the remaining 30%.
...
It seems that our model is dramatically accurate when classifying neighborhoods with excellent subway access (90%) and poor subway access (71.4%), while almost inaccurate when studying a neighborhood with typical subway access (9.09%). For neighborhoods with typical subway access, they are more likely to be categorized into excellent than typical or poor. For the remaining categories of transportation access, there was only a .02 difference in accuracy. Note that in both the ordinal and the naive model, we could not accurately predict this "Typical" category. 
...
```{r}
data_train %>% 
  ggplot(aes(x=perc_covered_by_transit)) + geom_histogram() + 
  labs(title="Density Plot of Percentage Covered by Transit", y="Count", x="% Covered by Transit") +
   theme(panel.grid.major.x = element_line("transparent"),
         # axis.text.y.left = element_blank(),
          axis.text.x.bottom = element_text(size = 12, face = "bold"),
          plot.title = element_text(size = 20, hjust=.5, face = "bold", family="DIN Condensed"),
...
df <- data.frame(c("25" = ecdf(data_train$perc_covered_by_transit)(25),
             "75" = ecdf(data_train$perc_covered_by_transit)(75)))
  colnames(df) <- c("Prop")

knitr::kable(df)
```

We investigated this issue with the density plot and the empirical CDF of the interval cutoffs. We found that the original data demonstrates a bimodal distribution with observations heavily concentrated in 0-1% and 99-100%. This is partially due to the over-saturated coverage in Manhattan and the lack of subway access in further, suburban neighborhoods in Queens. 
...
black_eviction <- nyc_compiled %>%
  ...
  facet_wrap(.~transportation_desert_3cat, scales = "free") 
