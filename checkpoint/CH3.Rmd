...
ppc_intervals(list, yrep = predictions_poisson,
              prob_outer = 0.8) +
...
draft %>%
  ...
  geom_sf()+
  scale_fill_gradientn(colors = c("#FCF5EE","#BD9DA5", "#9C7080", "#7B435B"),  guide = guide_legend(title = "Residual")) +
...
draft %>%
  ...
  ggplot(aes(fill = resid), color = "#8f98aa") +
  geom_sf()+
  scale_fill_gradientn(colors = c("#FCF5EE","#BD9DA5", "#9C7080", "#7B435B"),  
                       guide = guide_legend(title = "Number White")) +
  theme_minimal() +
  theme(panel.grid.major = element_line("transparent"),
        axis.text = element_blank()) +
...
nyc_predict_clean <- nyc_clean %>%
  na.omit()%>%
  arrange(assault_count)

list <- nyc_predict_clean %>%
  arrange(assault_count) %>% pull(assault_count)
...
nyc_predict_clean <- nyc_clean %>%
  na.omit()%>%
  arrange(sex_crime_count)

list <- nyc_predict_clean %>%
  arrange(sex_crime_count) %>% pull(sex_crime_count)
