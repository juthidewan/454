...
clean <- clean %>% 
  ...
  mutate(transportation_desert_3cat = cut(perc_covered_by_transit, breaks=c(-10, 0, 100, 500), labels = c("No Access", "Satisfactory Access", "Excellent Access"))) %>% 
  mutate(transportation_desert_4cat = cut(perc_covered_by_transit, breaks=c(-10, 0, 50, 100, 500), labels = c("No Access", "Limited Access", "Satisfactory Access", "Excellent Access")))

clean %>%write_csv("clean_data/nyc_names.csv")

a <- read_sf("clean_data/nyc_data.shp")
head(a)
b <- read_csv("clean_data/nyc_names.csv")
b %>% relocate(geometry, .after=transportation_desert_4cat) %>%
